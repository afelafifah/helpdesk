...
<div class="panel panel-primary">
  <div class="panel-heading">
    <h5 class="panel-title"><i class="icon-pencil7"></i> Detail Individu Di Tiket <b><i><?=getnamakk($id);?></i></b></h5>
  </div>
          ...
          <?php $no=0; foreach($all as $row): $no++; ?>
            <tr>
                  ...
                  <center>
                    <?php $num=cekKK($row->id_pegawai); 
                      if($num>0){
                        ...
                        echo '<a href="'.site_url('entry/tiket/'.$row->id_pegawai).'" class="btn btn-default btn-xs tooltips" data-popup="tooltip" data-original-title="Jadikan Kepala Keluarga" data-placement="top"><i class="icon-quill4"></i></a>';
                      }
                    ?>
                  </center>
                </td>
                ...
                <?php if($row->status==1){
                  ...
                  $site=site_url('entry/deadindividu/'.$row->id_pegawai);
                  $teks="Nonaktifkan Data";
                  $icon="switch";
                  $class="warning";
                }elseif($row->status==2){
                  echo "<span class='label label-danger'> Meninggal</span>";
                  $site=site_url('entry/onindividu/'.$row->id_pegawai);
                  $teks="Aktifkan Data";
                  $icon="switch";
                  $class="default";
                }else{
